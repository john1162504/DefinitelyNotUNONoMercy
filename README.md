...
# Integration smoke test (server must be running)
node server/scripts/simulate-3player.mjs
```

## Deploy / cold start

Set `VITE_SERVER_URL` and `CLIENT_URL` (CORS) on Render. Point the health check to `/health`. The client shows a startup overlay until `/health` responds.
