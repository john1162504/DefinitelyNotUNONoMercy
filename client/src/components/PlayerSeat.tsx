...
function PlayerSeat({
  ...
  `}
                ...
                {isYou && (
                    <span className="absolute -bottom-3 left-1/2 -translate-x-1/2 whitespace-nowrap text-[10px] font-bold uppercase tracking-wide bg-blue-600 text-white px-2 py-0.5 rounded-full shadow">
                        You
                    </span>
                )}
            </div>
            {cardCount !== undefined && (
                <span
                    className={`text-xs font-bold bg-blue-500 text-white px-1.5 rounded-full ${isYou ? "mt-3" : "mt-0.5"}`}
                >
                    {cardCount}
                </span>
            )}
